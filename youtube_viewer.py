...
from youtubeviewer.download_driver import copy_drivers
...
def control_player(driver, output, position, youtube, collect_id=True):
    ...
    create_html({"#3b8eea": f"Worker {position} | ",
                 "#23d18b":  f"--> {youtube} Found : {output} | Watch Duration : {duration} "})
...
def music_and_video(position, youtube, driver, output, view_stat):
    rand_choice = 1
    ...
    for i in range(rand_choice):
        if i == 0:
            current_url, current_channel = control_player(
                driver, output, position,  youtube, collect_id=True)
            ...
            current_url, current_channel = control_player(
                driver, output, position, youtube, collect_id=False)
...
def channel_or_endscreen(position, youtube, driver, view_stat, current_url, current_channel):
    option = 1
...
def main_viewer(position):
    ...
    try:
        ...
        try:
            ...
            driver = get_driver(viewports, agent)
            ...
            if 'watching' in view_stat:
                ...
                current_url, current_channel = music_and_video(
                     position, youtube, driver, output, view_stat)

            channel_or_endscreen(position, youtube, view_stat, current_url, current_channel)
...
def main():
    global cancel_all, threads, hash_config, futures, cpu_usage
    ...
    loop = 0
    pool_number = list(range(threads))

    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [executor.submit(main_viewer, position)
...
if __name__ == '__main__':
    ...
    while len(view) < views:
        try:
            ...
            # copy_drivers(patched_drivers=patched_drivers,
            #               total=max_threads)
            main()
